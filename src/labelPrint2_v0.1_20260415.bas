...
Sub 자동_양식_출력_20260415()
    ...
    Do While wsMain.Cells(row, "B").Value <> ""
        ...
        For idx = 1 To B1List.Count
            ...
            Dim rawB1 As String
            Dim printB1 As String
            Dim commaPos As Long
            Dim colonPos As Long
        
            rawB1 = Trim(CStr(B1List(idx)))
            shrinkCode = ShrinkCodeList(idx)
        
            ' 3조건: "뒤틀림" 포함 시 인쇄 안함
            If InStr(rawB1, "뒤틀림") > 0 Then
                GoTo NextIdx
            End If
            ...
            ' 1조건:
            ' 첫 번째 "," 다음 문자부터 ":" 전까지만 사용
            ' 예: "AAA, BBB : CCC" -> "BBB"
            commaPos = InStr(printB1, ",")
            colonPos = InStr(printB1, ":")
        
            If commaPos > 0 And colonPos > commaPos Then
                printB1 = Mid(printB1, commaPos + 1, colonPos - commaPos - 1)
            ElseIf commaPos > 0 Then
                ' ":"가 없으면 "," 다음부터 끝까지 사용
                printB1 = Mid(printB1, commaPos + 1)
            End If
        
            printB1 = Trim(printB1)
        
            ' 2조건: 가공 후 이전 값과 동일하면 인쇄 안함
            If prevPrintedB1 <> "" Then
                If printB1 = prevPrintedB1 Then
                    GoTo NextIdx
                End If
            End If
